...
from watserface.types import Anchors, Angle, BoundingBox, Distance, Face, FaceDetectorModel, FaceLandmark478, FaceLandmark5, FaceLandmark68, Mask, Matrix, Points, Scale, Score, Translation, VisionFrame, WarpTemplate, WarpTemplateSet
...
def create_normal_map(landmarks_478: FaceLandmark478, size: Size) -> VisionFrame:
	...
	if simplices is None:
		try:
			...
			# Validate triangulation before caching
			# Check max edge length relative to face span
			pts = landmarks_478[:, :2]
			tri_verts = pts[simplices]
			v0, v1, v2 = tri_verts[:, 0], tri_verts[:, 1], tri_verts[:, 2]
			d1 = numpy.linalg.norm(v1 - v0, axis=1).max()
			d2 = numpy.linalg.norm(v2 - v1, axis=1).max()
			d3 = numpy.linalg.norm(v0 - v2, axis=1).max()
			max_edge = max(d1, d2, d3)

			min_pt = pts.min(axis=0)
			max_pt = pts.max(axis=0)
			span = numpy.linalg.norm(max_pt - min_pt)
