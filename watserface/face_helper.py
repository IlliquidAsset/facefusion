...
_TRIANGULATION_CACHE : Dict[str, NDArray[Any]] = {}
...
def create_normal_map(landmarks_478: FaceLandmark478, size: Size) -> VisionFrame:
	...
	simplices = _TRIANGULATION_CACHE.get('478')
	...
	if simplices is None:
		try:
			...
			# Validity check: Max edge length < 0.5 * Face Span
			min_xy = landmarks_478[:, :2].min(axis=0)
			max_xy = landmarks_478[:, :2].max(axis=0)
			face_span = numpy.max(max_xy - min_xy)

			tris = landmarks_478[simplices]
			edge1 = numpy.linalg.norm(tris[:, 1] - tris[:, 0], axis=1)
			edge2 = numpy.linalg.norm(tris[:, 2] - tris[:, 1], axis=1)
			edge3 = numpy.linalg.norm(tris[:, 0] - tris[:, 2], axis=1)
			max_edge = max(edge1.max(), edge2.max(), edge3.max())

			if max_edge < 0.5 * face_span:
				_TRIANGULATION_CACHE['478'] = simplices
		except Exception:
			return numpy.zeros((height, width, 3), dtype=numpy.uint8)
