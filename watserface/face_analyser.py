...
from watserface.types import BoundingBox, Face, FaceDetectorModel, FaceLandmark5, FaceLandmarkSet, FaceScoreSet, Score, VisionFrame


def create_faces(vision_frame : VisionFrame, bounding_boxes : List[BoundingBox], face_scores : List[Score], face_landmarks_5 : List[FaceLandmark5], face_detector_model : Optional[FaceDetectorModel] = None, face_detector_angles : Optional[List[int]] = None, face_detector_score : Optional[float] = None, face_landmarker_score : Optional[float] = None) -> List[Face]:
	faces = []
	# Hoist state_manager lookups to prevent performance degradation from repetitive stack walking
...
def get_many_faces(vision_frames : List[VisionFrame], skip_cache : bool = False, face_detector_model : Optional[FaceDetectorModel] = None, face_detector_angles : Optional[List[int]] = None, face_detector_score : Optional[float] = None, face_landmarker_score : Optional[float] = None) -> List[Face]:
		...
		if numpy.any(vision_frame):
			...
			if static_faces:
				...
				if all_bounding_boxes and all_face_scores and all_face_landmarks_5 and face_detector_score > 0:
					faces = create_faces(vision_frame, all_bounding_boxes, all_face_scores, all_face_landmarks_5, face_detector_model, face_detector_angles, face_detector_score, face_landmarker_score)
